...
def takeProfitTo(bottom)
  if totalBalanceInUsd > bottom
    ...
    begin
      ...
      redis = Redis.new
      redis.set('ProfitTo', redis.get('ProfitTo').to_f + (redis.get('ProfitTo').to_f * 0.0005))
      puts ''
...
def balances
  ...
  balncs << {
    ...
    'split' => 1 - (redis.get('LTC_split').to_f + redis.get('BCH_split').to_f + redis.get('BTC_split').to_f + redis.get('ETH_split').to_f + redis.get('LINK_split').to_f)
  }
